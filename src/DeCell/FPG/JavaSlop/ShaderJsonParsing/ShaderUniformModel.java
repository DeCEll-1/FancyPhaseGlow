...
import DeCell.FPG.Frontend.Backend.Renderable.GlowRenderable;
...
public class ShaderUniformModel {
    ...
    public MyPanel createUniformModal(MyPanel parent, Ship ship) {
        ...
        if (this.type == UniformType.COL3) {
            new ColorPickerV2Dialogue().popup(
                    ...
                    FancyPhaseGlow.<Color>getShipProperty(ship, this.name),
                    colorPickerV2 -> {
            ...
            if (!empty) {
                ...
                deleteButton.addToInternalData("parent", parent)
                        ...
                        .addOnMouseDown(btn -> {
                            ...
                            FancyPhaseGlow.setShipProperty(_currShip, _uniform.name, getFloatMatrixFromColors(compactedColors));
    ...
    private static Object getValOrDefault(ShaderUniformModel uniform, ShipAPI ship, Sprite sprite) {
        ...
        if (uniform.arrayLengthReferenceTarget != null)
            return FancyPhaseGlow.getShipProperty(ship, "arrLen_" + uniform.name);

        Object val = isEngine ? uniform.getFromEngine(ship, sprite) : FancyPhaseGlow.getShipProperty(ship, uniform.name);
